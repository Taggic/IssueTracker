...
//    3. Output
        $found_tbl = '
          <table class="inline it_search_result">
            <tr><th>'.$this->getLang('search_Type').'</th><th>'.$this->getLang('search_ID').'</th><th>'.$this->getLang('search_Subject').'</th></tr>'.NL;
//     3.1 loop through reference_Array
        if($cnt_i_findings > -1) {
          foreach($ref_findings['issues'] as $item) {
              ...
              $text_snip = $this->xs_format($issues[$item['id']]['description']);
              ...
              $found_issues .= '<tr><td>'.$this->getLang('search_Issue').'</td>
                                    <td><a href="'.$link.'" title="'.$item['id'].'">'.$item['id'].'</a></td>
                                    <td><b><a href="'.$link.'" title="'.$item['id'].'">'.$issues[$item['id']]['title'].'</a></b><br />'.$text_snip.'</td>
                                </tr>'.NL;
          }
        }
        if($cnt_c_findings > -1) {
              ...
              foreach($comments as $comment) {
                  ...
                  if(stripos($comment['comment'],$needle)!==false) {
                      ...
                      $found_issues .= '<tr><td>'.$this->getLang('search_Comment').'</td>
                                            ...
                                            <td><a href="'.$link.'#a'.$comment['id'].'" title="'.$comment['id'].'">'.$text_snip.'</a></td>
                                        </tr>'.NL;
                  }  
              }
          }                               

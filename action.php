                       ...
                       if ($this->_captcha_ok())
                         {
                             if (checkSecurityToken())
                             {
                                 ...
                                 fclose($fh);
                                 $Generated_feedback = '<div style="border: 3px green solid; background-color: lightgreen; width: 87%; margin: 0px; padding: 10px;">Your comment has been successfully stored with ID #'.$comment_id.'</div>';
                                 // inform user (or assignee) about update
                                 $this->_emailForIssueMod($_REQUEST['project'],$_REQUEST['comment_issue_ID'], $comments[$comment_id]);
                                 
                                 // update $issues[$issueID]['modified'] date 

                                 // show this comment log update 
              ...
              if ($comments!=false) {              
                  foreach ($comments as $a_comment)
                  {
                        ...
                        $issue_comments_log .= '<TABLE width="85%" style="margin-left:10px; margin-right:10px">'.
                                               ...
                                               '<TR><TD>'.$this->_get_one_value($a_comment,'comment').'</TD></TR></TABLE><hr width="90%">';
                  }
              } 
